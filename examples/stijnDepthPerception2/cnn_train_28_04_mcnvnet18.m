...
nmb_batches = ceil(size(opts.train,1)/opts.batchSize);
loss_matrix = zeros(opts.numEpochs,nmb_batches,2);
...
if isstr(opts.errorFunction)
  switch opts.errorFunction
    ...
    case 'RMSE_linear'
      opts.errorFunction = @error_RMSElinear ;
      if isempty(opts.errorLabels), opts.errorLabels = {'RMSE_linear'} ; end
    otherwise
...
modelLossPath = fullfile(opts.expDir, sprintf('loss.mat'));
...
for epoch=start+1:opts.numEpochs
  ...
  if numGpus <= 1
    [net,stats.train,prof, loss_matrix(epoch,:,1)]= process_epoch(opts, getBatch, epoch, train, learningRate, imdb, net) ;
    [~,stats.val, ~, loss_matrix(epoch,:,2)] = process_epoch(opts, getBatch, epoch, val, 0, imdb, net) ;
    if opts.profile
      profile('viewer') ;
      keyboard ;
    end
    ...
    spmd(numGpus)
      [net_, stats_train_,prof_, loss_matrix(epoch,:,1)] = process_epoch(opts, getBatch, epoch, train, learningRate, imdb, net) ;
      [~, stats_val_, ~, loss_matrix(epoch,:,2)] = process_epoch(opts, getBatch, epoch, val, 0, imdb, net_) ;
    end
    net = net_{1} ;
  ...
  if ~evaluateMode && epoch==opts.numEpochs % only store values the last epoch
  ...
  savefig('data/coarse_depth/net-train.fig'); 
  drawnow ;
  print(1, modelFigPath, '-dpdf') ;
end
save(modelLossPath,'loss_matrix');
...
% -------------------------------------------------------------------------
function err = error_RMSElinear(opts, labels, res)
% -------------------------------------------------------------------------
predictions = gather(res(end-1).x) ;
predictions = reshape(predictions,55,74,[]);
err = bsxfun(@RMSE_linear, predictions, labels);

% -------------------------------------------------------------------------
function  [net_cpu,stats,prof,loss] = process_epoch(opts, getBatch, epoch, subset, learningRate, imdb, net_cpu)
% -------------------------------------------------------------------------

nmb_batches = ceil(size(opts.train,1)/opts.batchSize);
loss = zeros(nmb_batches,1);

modelLastItPath = fullfile(opts.expDir, sprintf('last_it.mat'));
...
%     loss_batch = mean(res(end).x(1,1,:)); 
%     loss(fix(t/opts.batchSize)+1) = loss_batch;
    loss_batch = mean(res(end).x(1,1,:)); 
    %loss(fix(t/opts.batchSize)+1) = loss_batch; 
    loss(fix(t/opts.batchSize)+1) = gather(loss_batch);%% JAY: replacing GPU array
    %with normal array    

%     if(training && epoch == opts.numEpochs && fix(t/opts.batchSize)+1 == nmb_batches)
%         figure;
%         subplot(1,3,1);
%         imagesc(im(:,:,:,1));
%         axis off;
...
%         axis off;
%         axis equal;
%         title('Training: Depth prediction');        
%     elseif(~training && fix(t/opts.batchSize)+1 == 1 ) % && epoch == opts.numEpochs
    if (~training && fix(t/opts.batchSize)+1 == 1 )
        figure;
        subplot(1,3,1);
        imagesc(uint8(im(:,:,:,1)));
        axis off;
        ...
        folder = 'data/coarse_depth/';
        baseFileName =  sprintf('validation1_epoch%d.fig', epoch);
        ...
        output = reshape(res(end-1).x(1,1,:,2),[55 74]);
        subplot(1,3,3);
        imagesc(output);
        axis off;
        axis equal;
        ...
        folder = 'data/coarse_depth/';
        baseFileName =  sprintf('validation2_epoch%d.fig', epoch);
        fullFileName = fullfile(folder, baseFileName);
        savefig(fullFileName);
        ...
        output = reshape(res(end-1).x(1,1,:,3),[55 74]);
        subplot(1,3,3);
        imagesc(output);
        axis off;
        axis equal;
        ...
        folder = 'data/coarse_depth/';
        baseFileName =  sprintf('validation3_epoch%d.fig', epoch);
        fullFileName = fullfile(folder, baseFileName);
        savefig(fullFileName);
        
        save(modelLastItPath,'res','im','labels');        
    end
    fprintf('Average loss over all images in the current batch: %d\n',loss_batch);
