function [net, stats] = cnn_train_control_output(net, imdb, getBatch, varargin)
...
for t=1:opts.batchSize:numel(subset)
  ...
  for s=1:opts.numSubBatches
    ...
    %% JAY: temp fix for res(end).x having some Inf values              
%     indices=find(res(end).x == Inf);
%     temp_arr = res(end).x;
%     temp_arr(indices) = 1.0;
%     res(end).x = temp_arr;
    %%
    ...
    if (~strcmp(mode, 'train') && fix(t/opts.batchSize)+1 == 1 )
	im_mean = state.imdb.images.image_mean;        

	figure;
        subplot(1,3,1);
        imagesc(uint8(im_mean+im(:,:,:,1)));
        axis off;
        axis equal;
        title('RGB');
        ...
        folder = opts.expDir;
        baseFileName =  sprintf('validation1_epoch%d.fig', state.epoch);
        fullFileName = fullfile(folder, baseFileName);
        savefig(fullFileName);
        
        figure;
        subplot(1,3,1);
        imagesc(uint8(im_mean+im(:,:,:,2)));
        axis off;
        axis equal;
        title('RGB');
        ...
        folder = opts.expDir;
        baseFileName =  sprintf('validation2_epoch%d.fig', state.epoch);
        fullFileName = fullfile(folder, baseFileName);
        savefig(fullFileName);
        
        figure;
        subplot(1,3,1);
        imagesc(uint8(im_mean+im(:,:,:,3)));
        axis off;
        axis equal;
        title('RGB');
        ...
        folder = opts.expDir;
        baseFileName =  sprintf('validation3_epoch%d.fig', state.epoch);
        fullFileName = fullfile(folder, baseFileName);
        savefig(fullFileName);    
    end
