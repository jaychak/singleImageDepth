...
if isstr(opts.errorFunction)
  switch opts.errorFunction
    ...
    case 'absRelativeDifference'
      opts.errorFunction = @error_absRelativeDifference ;
      if isempty(opts.errorLabels), opts.errorLabels = {'absRelativeDifference'} ; end
    case 'RMSE_linear'
...
for epoch=start+1:opts.numEpochs
  ...
  %temporarily giving nothing back
  info = [];
  % save
...
% -------------------------------------------------------------------------
function err = error_absRelativeDifference(opts, labels, res)
% -------------------------------------------------------------------------
predictions = gather(res(end-1).x) ;
predictions = reshape(predictions,55,74,[]);
error = bsxfun(@absRelativeDifference, predictions, labels);
err = sum(error(:)) ;
