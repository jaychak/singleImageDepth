function test_trained_cnn_image_sequence()
...
rgb_dir = '/esat/emerald/pchakrav/ICRAVideos/forestTrailsDataset/000/centre1/';%'/esat/emerald/pchakrav/ICRAVideos/castleVideo/images';%'/esat/emerald/pchakrav/corridorVideo/frames/';

depth_dir = '/esat/emerald/pchakrav/corridorVideo/depths/';
%% Process image in dir

num_frames = 722;

for frame_idx=1:num_frames
    %filename = fullfile(rgb_dir,strcat(sprintf('%04d',frame_idx),'.jpg'));
    filename = fullfile(rgb_dir,strcat(sprintf('%08d',frame_idx),'.jpg'));
    image = imread(filename);
    rgb_image = imresize(image, [240,320]);
    figure(1),imagesc(rgb_image);
    dzdy = [];
    res = [];

    res = vl_simplenn(net, single(rgb_image), dzdy, res, ... 
                  'mode', 'test', ...
    ...
    output1 = imresize(output,[240,320]);
    figure(2),imagesc(output1);
    pause(0.05);
